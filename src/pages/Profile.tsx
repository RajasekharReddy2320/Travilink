...
const Profile = () => {
  ...
  const loadFollowCounts = async (targetUserId: string) => {
    const [followersRes, followingRes] = await Promise.all([supabase.from("user_follows").select("*", {
      count: "exact",
      head: true
    }).eq("following_id", targetUserId), supabase.from("user_follows").select("*", {
      count: "exact",
      head: true
  ...
  const loadConnectionStatus = async (currentId: string, targetId: string) => {
    const {
      data
    } = await supabase.rpc('get_connection_status', {
      user1_id: currentId,
      user2_id: targetId
    });
    ...
    const {
      data: targetProfile
    } = await supabase.from("profiles").select("is_public").eq("id", targetId).single();
  ...
  const loadUserActivity = async (targetUserId: string) => {
    const {
      data: likes
    } = await supabase.from("trip_likes").select(`trip_id, trips:trip_id (*)`).eq("user_id", targetUserId);
    if (likes) {
      setLikedTrips(likes.map(l => l.trips).filter(Boolean));
    }
    const {
      data: bucket
    } = await supabase.from("bucket_list").select(`trip_id, trips:trip_id (*)`).eq("user_id", targetUserId);
    if (bucket) {
      setBucketList(bucket.map(b => b.trips).filter(Boolean));
    }
    const {
      data: trips
    } = await supabase.from("trips").select("*").eq("user_id", targetUserId).order("created_at", {
      ascending: false
    });
    if (trips) {
      setUserTrips(trips);
  ...
  const loadSocialActivity = async (targetUserId: string, currentId: string) => {
    const {
      data: posts
    } = await supabase.from("posts").select(`*, profiles:user_id (full_name, avatar_url)`).eq("user_id", targetUserId).order("created_at", {
      ascending: false
    });
    if (posts) {
      setUserPosts(posts);
    ...
    const {
      data: groups
    } = await supabase.from("travel_groups").select(`*, profiles:creator_id (full_name, avatar_url)`).eq("creator_id", targetUserId).order("created_at", {
      ascending: false
    });
    if (groups) {
      const groupsWithCounts = await Promise.all((groups || []).map(async (group: any) => {
        const {
          count
        } = await supabase.from("travel_group_members").select("*", {
          count: "exact",
          head: true
        }).eq("group_id", group.id).eq("status", "accepted");
        return {
          ...group,
    ...
    const {
      data: bookings
    } = await supabase.from("bookings").select("*").eq("user_id", targetUserId).order("created_at", {
      ascending: false
    });
    if (bookings) {
    ...
    if (currentId) {
      const [likesData, savesData, membershipsData] = await Promise.all([supabase.from("post_likes").select("post_id").eq("user_id", currentId), supabase.from("post_saves").select("post_id").eq("user_id", currentId), supabase.from("travel_group_members").select("group_id").eq("user_id", currentId).eq("status", "accepted")]);
      ...
      if (savesData.data) {
        ...
        if (savedPostIds.length > 0) {
          const {
            data: savedPostsData
          } = await supabase.from("posts").select(`*, profiles:user_id (full_name, avatar_url)`).in("id", savedPostIds).order("created_at", {
            ascending: false
          });
          if (savedPostsData) {
  ...
  const handleConnectionAction = async () => {
    ...
    if (connectionStatus === 'none') {
      ...
      const {
        error
      } = await supabase.from("user_connections").delete().eq("requester_id", currentUserId).eq("addressee_id", profile.id).eq("status", "pending");
      if (error) {
        toast({
          title: "Error",
      ...
      const {
        error
      } = await supabase.from("user_connections").delete().or(`and(requester_id.eq.${currentUserId},addressee_id.eq.${profile.id}),and(requester_id.eq.${profile.id},addressee_id.eq.${currentUserId})`).eq("status", "accepted");
      if (error) {
        toast({
          title: "Error",
          ...
          <div className="px-4 md:px-8 -mt-16 md:-mt-20">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-8">
              ...
              <div className="relative">
                ...
                {isOwnProfile && <button className="absolute bottom-1 right-1 p-2 rounded-full bg-primary text-primary-foreground shadow-lg hover:scale-105 transition-transform">
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <div className="flex flex-col md:flex-row items-center gap-4 mb-4">
                  ...
                  {isOwnProfile ? <div className="flex gap-2">
                      ...
                      {connectionStatus === 'pending_sent' && <Button variant="outline" size="sm" onClick={handleConnectionAction}>
                          Requested
                        </Button>}
                      {connectionStatus === 'connected' && <Button variant="secondary" size="sm" onClick={handleConnectionAction}>
                          <UserCheck className="mr-1 h-4 w-4" />
                          Following
                        </Button>}
                      {canMessage && <Button variant="outline" size="sm" onClick={() => navigate('/wanderlust', {
                    state: {
                ...
                {profile.home_location && <div className="flex items-center justify-center md:justify-start gap-1 mt-2 text-sm text-muted-foreground">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="posts" className="mt-6">
              {userPosts.length === 0 ? <div className="text-center py-16 bg-muted/30 rounded-2xl">
                ...
                </div> : <div className="grid grid-cols-3 gap-1 md:gap-3">
                  {userPosts.map(post => <div key={post.id} className="aspect-square rounded-lg overflow-hidden bg-muted relative group cursor-pointer">
                      {post.image_url ? <img src={post.image_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/20 to-accent/20 p-4">
            ...
            <TabsContent value="trips" className="mt-6 space-y-4">
              {userTrips.length === 0 ? <div className="text-center py-16 bg-muted/30 rounded-2xl">
                  ...
                  {userTrips.map(trip => <Card key={trip.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                      {trip.image_url && <img src={trip.image_url} alt={trip.title} className="w-full h-40 object-cover" />}
            ...
            <TabsContent value="groups" className="mt-6 space-y-4">
              {userGroups.length === 0 ? <div className="text-center py-16 bg-muted/30 rounded-2xl">
                  ...
                  {userGroups.map(group => <TravelGroupCard key={group.id} group={group} currentUserId={currentUserId} isMember={userGroupMemberships.has(group.id)} onUpdate={handleGroupUpdate} />)}
                </div>}
            </TabsContent>
            ...
            <TabsContent value="bookings" className="mt-6 space-y-4">
              {(() => {
              const now = new Date();
              const upcomingBookings = userBookings.filter(b => b.status === "confirmed" && new Date(b.departure_date) >= now);
              const completedBookings = userBookings.filter(b => b.status === "confirmed" && new Date(b.departure_date) < now);
              const cancelledBookings = userBookings.filter(b => b.status === "cancelled");
              const BookingCard = ({
                booking
              }: {
                booking: any;
              }) => <Card key={booking.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate('/ticket-details', {
                state: {
                  booking
                }
              })}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className={`p-3 rounded-xl ${booking.status === "cancelled" ? "bg-destructive/10" : new Date(booking.departure_date) < now ? "bg-green-500/10" : "bg-primary/10"}`}>
                            {booking.status === "cancelled" ? <XCircle className="h-5 w-5 text-destructive" /> : new Date(booking.departure_date) < now ? <CheckCircle className="h-5 w-5 text-green-600" /> : <Clock className="h-5 w-5 text-primary" />}
                          </div>
                          <div>
                        ...
                        <div className="text-right">
                          <Badge variant={booking.status === "cancelled" ? "destructive" : new Date(booking.departure_date) < now ? "secondary" : "default"}>
              ...
              return <Tabs defaultValue="upcoming" className="w-full">
                    ...
                    <TabsContent value="upcoming" className="space-y-3">
                      {upcomingBookings.length === 0 ? <div className="text-center py-12 bg-muted/30 rounded-2xl">
                        ...
                        </div> : upcomingBookings.map(booking => <BookingCard key={booking.id} booking={booking} />)}
                    </TabsContent>

                    <TabsContent value="completed" className="space-y-3">
                      {completedBookings.length === 0 ? <div className="text-center py-12 bg-muted/30 rounded-2xl">
                        ...
                        </div> : completedBookings.map(booking => <BookingCard key={booking.id} booking={booking} />)}
                    </TabsContent>

                    <TabsContent value="cancelled" className="space-y-3">
                      {cancelledBookings.length === 0 ? <div className="text-center py-12 bg-muted/30 rounded-2xl">
                        ...
                        </div> : cancelledBookings.map(booking => <BookingCard key={booking.id} booking={booking} />)}
                    </TabsContent>
                  </Tabs>;
            })()}
            </TabsContent>
            ...
            {isOwnProfile && <TabsContent value="saved" className="mt-6 space-y-6">
                {savedPosts.length === 0 ? <div className="text-center py-16 bg-muted/30 rounded-2xl">
                    ...
                    {savedPosts.map(post => <div key={post.id} className="aspect-square rounded-lg overflow-hidden bg-muted relative group cursor-pointer">
                        {post.image_url ? <img src={post.image_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/20 to-accent/20 p-4">
                            <p className="text-xs text-center line-clamp-4">{post.content}</p>
                          </div>}
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 text-white">
            ...
            <Card className="bg-gradient-to-br from-accent/5 to-primary/5 border-0">
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(star => <button key={star} type="button" onClick={() => setRating(star)} onMouseEnter={() => setHoveredRating(star)} onMouseLeave={() => setHoveredRating(0)} className="transition-transform hover:scale-110">
                      <Star className={`h-8 w-8 ${star <= (hoveredRating || rating) ? "fill-accent text-accent" : "text-muted-foreground"}`} />
                    </button>)}
                </div>
                <Textarea placeholder="Tell us about your experience..." value={reviewText} onChange={e => setReviewText(e.target.value)} rows={4} />
                <Button onClick={handleSubmitReview} disabled={isSubmittingReview || rating === 0 || reviewText.length < 10} className="w-full">
